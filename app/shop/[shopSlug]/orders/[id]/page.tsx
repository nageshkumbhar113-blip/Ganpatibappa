  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        <Link href={`${base}/orders`} className="text-gray-500 hover:text-gray-800"><ChevronLeft className="h-5 w-5" /></Link>
      ...
      <div className="max-w-xl mx-auto px-4 py-6 space-y-4">
        ...
        {order.status !== 'cancelled' ? (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="relative">
              ...
              <div className="absolute top-4 left-0 h-0.5 bg-orange-400 transition-all" style={{ width: currentStep < 0 ? '0%' : `${(currentStep / (STATUS_STEPS.length - 1)) * 100}%` }} />
                ...
                {STATUS_STEPS.map((step, i) => {
                  ...
                  return (
                    <div key={step} className="flex flex-col items-center gap-1">
                      <div className={`h-8 w-8 rounded-full border-2 flex items-center justify-center text-xs font-bold transition-colors ${isCompleted ? 'border-orange-500 bg-orange-500 text-white' : 'border-gray-200 bg-white text-gray-400'} ${isCurrent ? 'ring-2 ring-orange-200 ring-offset-2' : ''}`}>{i + 1}</div>
                      <span className={`text-[10px] font-medium text-center leading-tight ${isCompleted ? 'text-orange-600' : 'text-gray-400'}`}>{ORDER_STATUS_LABELS[step]}</span>
                    </div>
                  )
                })}
              </div>
          ...
          <div className="bg-red-50 rounded-xl border border-red-100 p-4 text-center"><p className="text-red-700 font-semibold">This order has been cancelled.</p></div>
        )}

        {/* Items */}
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-50"><h2 className="text-sm font-bold text-gray-900">Items Ordered</h2></div>
          <div className="divide-y divide-gray-50">
            {(order.order_items ?? []).map((item: any) => (
              <div key={item.id} className="flex items-center gap-3 px-4 py-3">
                <div className="h-12 w-12 shrink-0 rounded-lg overflow-hidden bg-orange-50">{item.products?.images?.[0] ? <img src={item.products.images[0]} alt={item.product_name} className="h-full w-full object-cover" /> : <div className="h-full flex items-center justify-center text-xl">🙏</div>}</div>
                <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-900">{item.product_name}</p><p className="text-xs text-gray-400">Qty: {item.quantity} × {formatCurrency(item.price)}</p></div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 col-span-2">
            ...
            <div className="flex items-center gap-1.5 mt-1 text-gray-500 text-sm"><Phone className="h-3.5 w-3.5" /><a href={`tel:${order.customer_phone}`} className="hover:text-orange-500">{order.customer_phone}</a></div>
            {order.customer_address && <div className="flex items-start gap-1.5 mt-1 text-gray-500 text-sm"><MapPin className="h-3.5 w-3.5 mt-0.5 shrink-0" /><span>{order.customer_address}</span></div>}
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4">
            ...
            {order.delivery_date && <><p className="text-xs text-gray-400 mt-1">Delivery</p><p className="text-xs font-semibold text-orange-600">{formatDate(order.delivery_date)}</p></>}
          </div>
        </div>
