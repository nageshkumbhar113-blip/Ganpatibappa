...
export default function WishlistPage() {
  ...
  useEffect(() => {
    const ids = getWishlistIds(shopSlug)
    if (ids.length === 0) {
      setIsLoading(false)
      return
    }
    apiFetch(`/api/shop/products?ids=${ids.join(',')}`)
      .then((r) => r.json())
  ...
  function removeItem(productId: string) {
    removeFromWishlist(shopSlug, productId)
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        {items.length === 0 ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            {items.map((p: any) => (
              <div key={p.id} className="group relative rounded-2xl overflow-hidden border border-gray-100 bg-white shadow-sm hover:shadow-md transition-shadow">
