...
export default function MyDetailsPage() {
  ...
  useEffect(() => {
    setForm(getCustomerDetails(shopSlug))
  }, [shopSlug])

  function handleSave(e: React.FormEvent) {
    e.preventDefault()
    saveCustomerDetails(shopSlug, form)
    setSaved(true)
    toast.success('जतन झालं — पुढच्या order मध्ये आपोआप भरलं जाईल')
    ...
    <div className="min-h-screen bg-gray-50 pb-16">
      <div className="max-w-md mx-auto px-4 py-8">
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-gray-500 mt-1">पुढच्या वेळी checkout आपोआप भरलं जाईल</p>
        </div>
        ...
        <form onSubmit={handleSave} className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 space-y-4">
          ...
          >
            <Save className="h-4 w-4" /> {saved ? 'जतन झालं ✅' : 'जतन करा'}
          </button>
        </form>
      </div>
    </div>
