...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      const [productRes, shopRes] = await Promise.all([
        apiFetch(`/api/shop/products?slug=${params.slug}&limit=1`),
      ...
      if (productRes.ok) {
        ...
        setProduct(p)
        if (p) {
          apiFetch('/api/shop/recently-viewed', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ product_id: p.id }) }).catch(() => {})
          if (p.category_id) {
  ...
  if (isLoading) return <div className="flex items-center justify-center min-h-screen"><Loader2 className="h-8 w-8 animate-spin text-orange-400" /></div>
  if (!product) return (
    <div className="min-h-screen flex flex-col items-center justify-center gap-3 text-gray-400">
      <span className="text-5xl">🙏</span><p>Product not found</p>
      <Link href={`${basePath}/products`} className="text-orange-500 hover:underline text-sm">View all products</Link>
    </div>
  )
  ...
  const waText = encodeURIComponent(`Hello, I'm interested in ordering *${product.name}*${product.height_cm ? ` (${product.height_cm} cm)` : ''}. Please confirm availability.`)

  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      <div className="sticky top-0 z-20 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        <button onClick={() => router.back()} className="text-gray-500 hover:text-gray-800"><ChevronLeft className="h-5 w-5" /></button>
        <h1 className="text-sm font-medium text-gray-900 line-clamp-1 flex-1">{product.name}</h1>
        <button onClick={() => { if (navigator.share) { navigator.share({ title: product.name, url: window.location.href }) } else { navigator.clipboard.writeText(window.location.href); toast.success('Link copied!') } }} className="text-gray-500 hover:text-gray-800">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          <div className="space-y-3">
            <div className="aspect-square rounded-2xl overflow-hidden bg-orange-50">
              {images[selectedImage] ? <img src={images[selectedImage]} alt={product.name} className="h-full w-full object-cover" /> : <div className="h-full flex items-center justify-center text-8xl">🙏</div>}
            </div>
            {images.length > 1 && (
              <div className="flex gap-2 overflow-x-auto pb-1">
                {images.map((img, i) => (
                  <button key={i} onClick={() => setSelectedImage(i)} className={`shrink-0 h-16 w-16 rounded-lg overflow-hidden border-2 transition-colors ${selectedImage === i ? 'border-orange-500' : 'border-transparent'}`}>
          ...
          <div className="space-y-4">
            <div>
              ...
              {product.categories?.name && <p className="text-sm text-orange-500 mt-1">{product.categories.name}</p>}
            </div>
            <div className="grid grid-cols-2 gap-2 text-sm">
              {product.height_cm && <div className="rounded-lg bg-orange-50 p-3"><p className="text-xs text-gray-400">Height</p><p className="font-semibold text-gray-800">{product.height_cm} cm</p></div>}
              {product.material && <div className="rounded-lg bg-orange-50 p-3"><p className="text-xs text-gray-400">Material</p><p className="font-semibold text-gray-800">{product.material}</p></div>}
              {product.stock !== null && <div className={`rounded-lg p-3 ${product.stock === 0 ? 'bg-red-50' : 'bg-green-50'}`}><p className="text-xs text-gray-400">Stock</p><p className={`font-semibold ${product.stock === 0 ? 'text-red-600' : 'text-green-700'}`}>{product.stock === 0 ? 'Out of Stock' : `${product.stock} available`}</p></div>}
            </div>
            {showPrice && (
              <div className="flex items-baseline gap-3">
                <span className="text-3xl font-bold text-gray-900">{formatCurrency(product.offer_price ?? product.price)}</span>
                {product.offer_price && <><span className="text-lg text-gray-400 line-through">{formatCurrency(product.price)}</span><span className="rounded-full bg-green-100 text-green-700 text-sm font-bold px-2 py-0.5">{discount}% OFF</span></>}
              </div>
            )}
            {product.stock !== 0 && (
              <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-1 rounded-full border border-gray-200 bg-white">
                  <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="h-8 w-8 flex items-center justify-center text-gray-600"><Minus className="h-4 w-4" /></button>
                  <span className="w-8 text-center text-sm font-semibold">{quantity}</span>
                  <button onClick={() => setQuantity(quantity + 1)} className="h-8 w-8 flex items-center justify-center text-gray-600"><Plus className="h-4 w-4" /></button>
                </div>
              </div>
            )}
            <div className="flex gap-2">
              <button onClick={toggleWishlist} className={`rounded-xl p-3 border transition-colors ${isWishlisted ? 'border-red-200 bg-red-50 text-red-500' : 'border-gray-200 text-gray-400 hover:border-red-200 hover:text-red-500'}`}>
                <Heart className={`h-5 w-5 ${isWishlisted ? 'fill-current' : ''}`} />
              </button>
              {product.stock !== 0 && (
                <>
                  <button onClick={handleAddToCart} className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-gray-900 py-3 text-sm font-semibold text-white hover:bg-gray-800 transition-colors">
                    <ShoppingCart className="h-4 w-4" />Add to Cart
                  </button>
                  <button onClick={() => setShowBookForm(true)} className="flex-1 rounded-xl bg-orange-500 py-3 text-sm font-semibold text-white hover:bg-orange-600 transition-colors">Book Now</button>
                </>
              )}
            </div>
            {whatsapp && (
        ...
        {reviews.length > 0 && (
          <div className="mt-6 rounded-xl bg-white border border-gray-100 shadow-sm p-5">
            <h2 className="text-base font-bold text-gray-900 mb-4">Reviews ({reviews.length})</h2>
            <div className="space-y-4">
              {reviews.map((r: any) => (
                <div key={r.id} className="border-b border-gray-50 last:border-0 pb-4 last:pb-0">
                  <div className="flex items-center gap-2">
                    <div className="flex">{Array.from({ length: 5 }).map((_, i) => <Star key={i} className={`h-3.5 w-3.5 ${i < r.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-200'}`} />)}</div>
        ...
        {related.length > 0 && (
          <div className="mt-8">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {related.map((rp: any) => (
                <Link key={rp.id} href={`${basePath}/products/${rp.slug}`} className="group rounded-xl overflow-hidden border border-gray-100 bg-white shadow-sm hover:shadow-md transition-shadow">
                  <div className="h-32 bg-orange-50 overflow-hidden">{rp.images?.[0] ? <img src={rp.images[0]} alt={rp.name} className="h-full w-full object-cover group-hover:scale-105 transition-transform" /> : <div className="h-full flex items-center justify-center text-3xl">🙏</div>}</div>
                  <div className="p-2.5"><p className="text-xs font-medium text-gray-900 line-clamp-2">{rp.name}</p>{showPrice && <p className="mt-1 text-xs font-bold text-gray-800">{formatCurrency(rp.offer_price ?? rp.price)}</p>}</div>
                </Link>
              ))}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl bg-white p-5 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleBookNow} className="space-y-3">
              {[
                { label: 'Your Name *', field: 'customer_name', type: 'text', required: true, placeholder: 'Ramesh Patil' },
